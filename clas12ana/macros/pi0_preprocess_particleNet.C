int pi0_preprocess_particleNet(
  ...
  tOut->Branch("isEle",&_isEle,"isEle[nPart]/I");
  tOut->Branch("isPositron",&_isPositron,"isPositron[nPart]/I");
  tOut->Branch("isGamma",&_isGamma,"isGamma[nPart]/I");
  tOut->Branch("isProton",&_isProton,"isProton[nPart]/I");
  tOut->Branch("isNeutron",&_isNeutron,"isNeutron[nPart]/I");
  tOut->Branch("isPion",&_isPion,"isPion[nPart]/I");
  tOut->Branch("isKaon",&_isKaon,"isKaon[nPart]/I");
  ...
  while(tr.Next()){
	...
	for(int j = 0 ; j < _nPart; j++){
	  ...
	  _pid[j] = pid;
      _isEle[j] = (pid==11)?1:0;
      _isPositron[j] = (pid==-11)?1:0;
      _isPion[j] = (pid==211 || pid==-211)?1:0;
      _isGamma[j] = (pid==22)?1:0;
      _isProton[j] = (pid==2212 || pid==-2212)?1:0;
      _isNeutron[j] = (pid==2112 || pid==-2112)?1:0;
      _isKaon[j] = (pid==321 || pid==-321)?1:0;
	  _eta[j] = eta;
	  _phi[j] = phi;
	  _vz[j] = vz;
	  _chi2[j] = chi2;
